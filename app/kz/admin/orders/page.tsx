...
export default function AdminOrdersPage() {
  ...
  return (
    <main className="p-10">
      <h1 className="text-2xl font-bold mb-6">Тапсырыстар</h1>

      <div className="mb-4">
        <label className="mr-2 font-medium">Күй бойынша сүзгі:</label>
        <select
          value={statusFilter}
        ...
        >
          ...
          <option value="в обработке">Жаңа</option>
          <option value="paid">Төленді</option>
          <option value="в пути">Жолда</option>
          <option value="завершён">Аяқталған</option>
          <option value="отменён">Бас тартылды</option>
        </select>
      </div>
        ...
        <table className="w-full text-sm border bg-white">
          <thead className="bg-gray-100">
            <tr>
              ...
              <th className="border px-2 py-1">Аты</th>
              <th className="border px-2 py-1">Телефон</th>
              <th className="border px-2 py-1">Мекенжай</th>
              <th className="border px-2 py-1">Сома</th>
              <th className="border px-2 py-1">Күйі</th>
              <th className="border px-2 py-1">Күні</th>
              <th className="border px-2 py-1">Әрекет</th>
            </tr>
          </thead>
          <tbody>
            {filteredOrders.map((order) => (
              <React.Fragment key={order.id}>
                <tr className="border-b">
                  ...
                  <td className="border px-2 py-1">{order.profiles?.email || 'белгісіз'}</td>
                  ...
                  <td className="border px-2 py-1">
                    ...
                    >
                      {expandedOrderId === order.id ? 'Жасыру' : 'Толығырақ'}
                    </button>
                  </td>
                </tr>
                ...
                {expandedOrderId === order.id && orderItems[order.id] && (
                  <tr className="bg-gray-50">
                    <td colSpan={9} className="p-4">
                      <h4 className="font-semibold mb-2">Тапсырыстағы тауарлар:</h4>
                      <ul className="space-y-1">
                        {orderItems[order.id].map((item, index) => (
                          <li key={index} className="text-sm">
                            {item.model} — {item.quantity} дана × {item.price} ₸
                          </li>
                        ))}
                      </ul>
                    </td>
