...
export default function Login() {
  ...
  const handleLogin = async () => {
    ...
    if (loginError) {
      setError('Кіру қатесі: ' + loginError.message)
    } else {
      if (email === 'izbasarraimbek373@gmail.com') {
        router.push('/kz/admin')
      } else {
        router.push('/kz/client/catalog')
      }
    }
  }
        ...
        <div className="max-w-md w-full">
          ...
          <div className="space-y-4">
            ...
            <div className="text-center mt-4 text-sm">
              Аккаунтыңыз жоқ па?{' '}
              <Link href="/loginSign/signup" className="text-indigo-600 hover:underline">
                Тіркеліңіз
              </Link>
            </div>
          </div>
